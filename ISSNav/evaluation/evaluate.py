...
#
# Easy  — adjacent modules, at most 1 junction between them
# Hard  — distant modules, requires navigating through 2+ junctions
#
# Based on ISS topology:
#   Main corridor (left to right): nauka-zvezda-zarya-unity-destiny-harmony-columbus
#   Vertical branches: poisk(off zvezda), rassvet(off zarya),
#                      tranquility/leonardo(off unity), kibo_elm/kibo_pm(off harmony)
# ---------------------------------------------------------------------------

EASY_PAIRS = [
    ("nauka",       "zvezda"),      # direct neighbours on main corridor
    ("zvezda",      "zarya"),
    ("zarya",       "unity"),
    ("unity",       "destiny"),
    ("destiny",     "harmony"),
    ("harmony",     "columbus"),
    ("zvezda",      "poisk"),       # vertical branch, 1 hop
    ("zarya",       "rassvet"),
    ("unity",       "tranquility"),
...
HARD_PAIRS = [
    ("nauka",       "columbus"),    # full main corridor traversal
    ("nauka",       "kibo_pm"),     # Russian seg → Japanese lab (top)
    ("prichal",     "columbus") if False else ("zvezda", "harmony"),  # long corridor
    ("rassvet",     "kibo_pm"),     # bottom branch → top branch
    ("leonardo",    "columbus"),    # bottom US → far right
    ("poisk",       "leonardo"),    # top Russian → bottom US
    ("nauka",       "tranquility"), # far left → US vertical branch
    ("kibo_pm",     "nauka"),       # top Japanese → far Russian left
    ("rassvet",     "poisk"),       # bottom Zarya → top Zvezda (cross+vertical)
    ("leonardo",    "kibo_pm"),     # bottom US → top Japanese
]
...
# ---------------------------------------------------------------------------
def evaluate_on_pairs(model, env, pairs, label="Model", use_greedy=False):
    """
    Runs one episode per (start, goal) pair and aggregates results.
    Returns success_rate, mean_reward, mean_steps, rewards, steps lists.
    """
    successes = 0
    rewards   = []
    steps_list = []
...
# ---------------------------------------------------------------------------
# Standard evaluate_policy (random seeds, full 100 episodes)
# ---------------------------------------------------------------------------
...
def plot_reward_distribution(ppo_rewards, sac_rewards, greedy_rewards,
    ...
    ax.hist(ppo_rewards,    bins=bins, alpha=0.6, label='PPO',
            color='steelblue', edgecolor='white')
    ax.hist(sac_rewards,    bins=bins, alpha=0.6, label='SAC',
...
def plot_summary(results, title="ISSNav-v0 Evaluation Summary",
                 fname="evaluation_summary.png"):
...
def plot_steps_distribution(ppo_steps, sac_steps, greedy_steps,
                             ...
                             fname="steps_distribution.png"):
    fig, ax = plt.subplots(figsize=(10, 5))
    bins = np.linspace(0, 700, 35)
    ax.hist(greedy_steps, bins=bins, alpha=0.5, label='Greedy Baseline',
            color='gray', edgecolor='white')
    ax.hist(ppo_steps,    bins=bins, alpha=0.6, label='PPO',
            color='steelblue', edgecolor='white')
    ax.hist(sac_steps,    bins=bins, alpha=0.6, label='SAC',
            color='tomato', edgecolor='white')
...
def plot_easy_hard_comparison(easy_results, hard_results,
                               fname="easy_vs_hard_comparison.png"):
    """Bar chart comparing easy vs hard performance for each model."""
    ...
    plt.suptitle('ISSNav-v0: Easy vs Hard Goal Difficulty Analysis',
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    # PART 1 — Overall evaluation (100 random episodes)
    # -----------------------------------------------------------------------
    print("\n" + "="*60)
    ...
    ppo_sr, ppo_mr, ppo_ms, ppo_r, ppo_s = evaluate_policy(
        ppo_model, env, n_episodes=N_EVAL_EPISODES, label="PPO")

    sac_sr, sac_mr, sac_ms, sac_r, sac_s = evaluate_policy(
        sac_model, env, n_episodes=N_EVAL_EPISODES, label="SAC")
    ...
    print(f"{'PPO':<18} {float(ppo_sr):>9.1f}% {float(ppo_mr):>10.3f} {float(ppo_ms):>10.1f}")
    print(f"{'SAC':<18} {float(sac_sr):>9.1f}% {float(sac_mr):>10.3f} {float(sac_ms):>10.1f}")
    print(f"{'='*55}")

    results = [
        {"label": "Greedy", "success_rate": greedy_sr,
         "mean_reward": greedy_mr, "mean_steps": greedy_ms},
        {"label": "PPO",    "success_rate": ppo_sr,
         "mean_reward": ppo_mr,    "mean_steps": ppo_ms},
        {"label": "SAC",    "success_rate": sac_sr,
    ...
    # -----------------------------------------------------------------------
    # PART 2 — Easy vs Hard goal evaluation
    # -----------------------------------------------------------------------
    print("\n" + "="*60)
    print("PART 2: Easy vs Hard Goal Difficulty Analysis")
    print("="*60)

    print(f"\nEasy pairs ({len(EASY_PAIRS)} pairs — adjacent modules):")
    ...
    print(f"\nHard pairs ({len(HARD_PAIRS)} pairs — distant modules, 2+ junctions):")
    ...
    print(f"{'PPO':<18} {float(ppo_easy_sr):>9.1f}% {float(ppo_easy_ms):>12.1f} {float(ppo_hard_sr):>9.1f}% {float(ppo_hard_ms):>12.1f}")
    print(f"{'SAC':<18} {float(sac_easy_sr):>9.1f}% {float(sac_easy_ms):>12.1f} {float(sac_hard_sr):>9.1f}% {float(sac_hard_ms):>12.1f}")
    print(f"{'='*65}")
